...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        },
        {
            ...
            description: 'Post a video with description to a Facebook page',
            inputSchema: {
                type: 'object',
                properties: {
                    pageId: {
                    ...
                    videoUrl: {
                        type: 'string',
                        description: 'URL of the video to post (must be publicly accessible)',
                    },
                    message: {
                        type: 'string',
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'facebook_post_video':
                const videoResult = await fbClient.uploadVideo(String(args.pageId), {
                    url: String(args.videoUrl),
                    message: args.message ? String(args.message) : '',
                });
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Video posted successfully! Video ID: ${videoResult.id}`,
                        },
                    ],
                };
